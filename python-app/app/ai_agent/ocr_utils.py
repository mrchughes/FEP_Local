...
def extract_text_from_image(image_path):
    ...
    try:
        ...
        logging.info(f"[OCR] Successfully extracted text from image: {image_path}")
        return text
    except Exception as e:
        logging.error(f"[OCR] Error extracting text from image: {e}")
        return ""
...
def extract_text_from_pdf(pdf_path):
    ...
    """
    try:
        logging.info(f"[OCR] Processing PDF file: {pdf_path}")
        extracted_text = ""
        ...
        if len(extracted_text.strip()) > 100:  # Threshold can be adjusted
            logging.info(f"[OCR] Successfully extracted text directly from PDF: {pdf_path}")
            return extracted_text
            
        # Otherwise, use OCR on the PDF
        logging.info(f"[OCR] Using OCR for PDF (insufficient text layer): {pdf_path}")
        ...
        logging.info(f"[OCR] Successfully extracted text from PDF using OCR: {pdf_path}")
        return extracted_text
    except Exception as e:
        logging.error(f"[OCR] Error extracting text from PDF: {e}")
        return ""
...
def pdf_image_conversion(pdf_path):
    ...
    """
    try:
        logging.info(f"[OCR] Converting PDF to images: {pdf_path}")
        ...
        logging.error(f"[OCR] Error converting PDF to images: {e}")
        return []
...
def extract_text_from_docx(docx_path):
    ...
    """
    try:
        logging.info(f"[OCR] Processing DOCX file: {docx_path}")
        text = docx2txt.process(docx_path)
        logging.info(f"[OCR] Successfully extracted text from DOCX: {docx_path}")
        return text
    except Exception as e:
        logging.error(f"[OCR] Error extracting text from DOCX: {e}")
        return ""
...
def process_document(file_path):
    ...
    logging.info(f"[OCR] Processing document with extension: {ext}")
    ...
    if ext in ['.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.tif']:
        ...
        logging.warning(f"[OCR] Unsupported file format: {ext}")
